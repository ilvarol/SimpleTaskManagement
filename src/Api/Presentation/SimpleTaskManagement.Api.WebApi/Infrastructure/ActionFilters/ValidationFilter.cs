using Microsoft.AspNetCore.Mvc.Filters;
using Microsoft.AspNetCore.Mvc;
using SimpleTaskManagement.Common.Infrastructure.Results;

namespace SimpleTaskManagement.Api.WebApi.Infrastructure.ActionFilters;

public class ValidationFilter
{
    public class ValidateModelStateFilter : IAsyncActionFilter
    {
        public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
        {
            if (!context.ModelState.IsValid)
            {
                var messages = context.ModelState.Values.SelectMany(x => x.Errors)
                                                        .Select(x => !string.IsNullOrEmpty(x.ErrorMessage) ?
                                                                x.ErrorMessage : x.Exception!.Message)
                                                        .Distinct()
                                                        .ToList();

                var res = new ValidationResponseModel(messages);
                context.Result = new BadRequestObjectResult(res);

                return;
            }

            await next();
        }
    }
}
